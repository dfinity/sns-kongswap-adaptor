...
use sns_treasury_manager::{
    Allowance, Asset, BalanceBook, DepositRequest, TreasuryManagerArg, TreasuryManagerInit,
    WithdrawRequest,
};
...
pub(crate) const FEE_ICP: u64 = 10_000;
pub(crate) const FEE_SNS: u64 = 10_000;
pub(crate) const E8: u64 = 100_000_000;

lazy_static! {
    pub(crate) static ref SNS_LEDGER_CANISTER_ID: Principal =
        Principal::from_text("jg2ra-syaaa-aaaaq-aaewa-cai").unwrap();

    pub(crate) static ref SNS_ROOT_CANISTER_ID: Principal =
        Principal::from_text("ju4gz-6iaaa-aaaaq-aaeva-cai").unwrap();

    pub(crate) static ref SNS_GOVERNANCE_CANISTER_ID: Principal =
        Principal::from_text("jt5an-tqaaa-aaaaq-aaevq-cai").unwrap();

    pub(crate) static ref ICP_LEDGER_CANISTER_ID: Principal =
        Principal::from_text("ryjl3-tyaaa-aaaaa-aaaba-cai").unwrap();

    pub(crate) static ref NNS_ROOT_CANISTER_ID: Principal =
        Principal::from_text("r7inp-6aaaa-aaaaa-aaabq-cai").unwrap();

    pub(crate) static ref NNS_GOVERNANCE_CANISTER_ID: Principal =
        Principal::from_text("rrkah-fqaaa-aaaaa-aaaaq-cai").unwrap();

    // Canister ID from the mainnet.
    // See https://dashboard.internetcomputer.org/canister/2ipq2-uqaaa-aaaar-qailq-cai
    pub(crate) static ref KONGSWAP_BACKEND_CANISTER_ID: Principal =
    ...
    pub(crate) static ref TREASURY_ICP_ACCOUNT: sns_treasury_manager::Account = sns_treasury_manager::Account {
        owner: *SNS_GOVERNANCE_CANISTER_ID,
        subaccount: None, // No subaccount for the SNS treasury.
    };

    pub(crate) static ref TREASURY_SNS_ACCOUNT: sns_treasury_manager::Account = sns_treasury_manager::Account {
        owner: *SNS_GOVERNANCE_CANISTER_ID,
...
pub(crate) async fn get_balance(
    ...
    account: sns_treasury_manager::Account,
) -> Nat {
    let request = Account {
        owner: account.owner,
    ...
    agent.call(icrc1_ledger_canister_id, request).await.unwrap()
}

pub(crate) async fn create_kong_adaptor(pocket_ic: &PocketIc, subnet_id: Principal) -> Principal {
...
pub(crate) fn get_kong_adaptor_wasm() -> Wasm {
...
pub(crate) async fn install_kong_adaptor(
    pocket_ic: &PocketIc,
    wasm: Wasm,
...
pub(crate) async fn install_kong_swap(pocket_ic: &PocketIc) {
    // Install KongSwap
...
pub(crate) async fn mint_tokens<Agent>(
    agent: &mut Agent,
    icrc1_ledger_canister_id: Principal,
...
pub(crate) async fn install_icp_ledger(pocket_ic: &PocketIc) -> Principal {
...
pub(crate) async fn setup_kongswap_adaptor(
    agent: &mut PocketIcAgent,
    sns_ledger_canister_id: Principal,
    icp_ledger_canister_id: Principal,
    kong_adaptor_canister_id: Principal,
    wasm: &Wasm,
    initial_deposit_sns: u64,
    initial_deposit_icp: u64,
) {
    mint_tokens(
